...
<aside class="glass-strong card-hairline p-6 flex flex-col gap-7 h-full">
  <header class="space-y-1">
    <span class="pill">paramètres</span>
    <h2 class="text-pearl text-xl font-semibold tracking-tight">Géométrie</h2>
    <p class="text-ash text-sm">
      Définissez les diamètres et l’angle. Les calculs Rust s’exécutent à chaque
      modification — tout est mesuré en millimètres.
    </p>
  </header>
  ...
  <section class="space-y-6">
    ...
    {/if}
    <Slider
      label={p.mode === "cyl_cyl" ? "Angle entre les axes" : "Inclinaison du plan"}
      unit="°"
  ...
  {#if p.mode === "cyl_cyl"}
    <section class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <span class="text-[10px] text-ash/70">choix de la lèvre conservée</span>
      </div>
      <Segmented
  ...
  <footer class="mt-auto pt-4 border-t border-white/5 space-y-2 text-[11px] text-ash">
