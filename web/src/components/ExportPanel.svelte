...
<div class="flex flex-col gap-6">
  ...
  <section class="space-y-2">
    ...
    <div class="grid grid-cols-2 gap-2">
      {#each [["title", "Titre"], ["project", "Projet"], ["author", "Auteur"], ["date", "Date"]] as [key, ph] (key)}
        <input
          type="text"
          placeholder={ph}
          class="bg-black/40 border border-mist/50 rounded-md px-2.5 py-1.5 text-[13px] text-pearl placeholder:text-ash/60 focus:outline-none focus:border-ember/70 transition-colors min-w-0"
    ...
    <input
      ...
      class="w-full bg-black/40 border border-mist/50 rounded-md px-2.5 py-1.5 text-[13px] text-pearl placeholder:text-ash/60 focus:outline-none focus:border-ember/70 transition-colors"
        ...
        {#each editor.annotations as a, index (index)}
          <li class="flex items-center gap-1.5">
            <span
              ...
              style="background: {a.pattern === 'branch' ? '#ff5b1a' : '#29c2ff'}"
              title={a.pattern === "branch" ? "tube incliné" : "gueule de loup"}
            ></span>
            <input
              type="text"
              class="flex-1 min-w-0 bg-black/40 border rounded-md px-2 py-1 text-[12px] text-pearl focus:outline-none transition-colors {editor.selected ===
              index
                ? 'border-ember/80'
  ...
  <section class="pt-3 border-t border-white/5 space-y-2">
